...
class ScriptArguments:
    ...
    """
    dataset_path: Optional[str] = field(default="/opt/ml/input/data/training", metadata={"help": "Traninig data path"})
    # NOTE: gpt2 models use Conv1D instead of Linear layers which are not yet supported in 8 bit mode
    # models like gpt-neo* models are more suitable.
    model_name: Optional[str] = field(
        default="/opt/ml/input/data/pretrained-model", metadata={"help": "the model name"}
    )
...
print(f"dataset_path: {script_args.dataset_path}")
print(f"model_path: {script_args.model_name}")
print(f"dataset_list: {os.listdir(script_args.dataset_path)}")
print(f"model_path_list: {os.listdir(script_args.model_name)}")    
print("-------------------------------------")
